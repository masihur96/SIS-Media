...
class PrintManagmentAllData extends StatefulWidget {
  const PrintManagmentAllData({Key? key}) : super(key: key);

  @override
  _PrintManagmentAllDataState createState() => _PrintManagmentAllDataState();
}

class _PrintManagmentAllDataState extends State<PrintManagmentAllData> {
  bool _isLoading = false;
  List channels = Variables().getPrintManagmentList();
  String channelValue = 'MANAGING BOARD OF BANGLADESH PRESS INSTITUTE';
    ...
    if (fatchDataHelper.managementDataList.isEmpty) {
      ...
      await fatchDataHelper.fetchManagementData().then((value) {
        setState(() {
          ...
          _filteredList = _subList;
          _filterChannelList('MANAGING BOARD OF BANGLADESH PRESS INSTITUTE');
          _isLoading = false;
        });
      });
    } else {
